...
document.addEventListener('DOMContentLoaded', function () {
  ...
  loginForm.addEventListener('submit', function (e) {
    ...
    if ((email === 'edgar.visurraga@sacmigroup.com' && password === '1234') ||
      ...
      nombrePDF = lugar + fechaHoraFormato + '.pdf';
    } else {
  ...
  webSimForm.addEventListener('submit', async function (e) {
    ...
    if (confirm('¿Está seguro de que desea enviar el formulario?')) {
      ...
      try {
        doc.save(nombrePDF);  // Nombre del archivo que se descargará en el sistema del usuario
      } catch (err) {
  ...
  async function uploadPDFToGitHub(doc) {
    ...
    const owner = 'EdgarVN';
    const path = 'Acta/' + nombrePDF;  // Ruta en el repositorio
